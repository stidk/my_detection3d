...
class PillarFeatureNet(nn.Module):
    ...
    def forward(self, features, num_points, coors):
        ...
        if self._scanid:
            ...
            # pointcloud = np.fromfile(str("/home/dk/kitti_dataset/training/velodyne_reduced/000001.bin"),
            #                          dtype=np.float32, count=-1).reshape([-1, 4])
            pointcloud = features
            x = pointcloud[:, :, 0]
            y = pointcloud[:, :, 1]
            z = pointcloud[:, :, 2]
            angle = ((torch.arctan(z / torch.sqrt(x * x + y * y)) * 180) / np.pi).view(-1,32,1)
            # print(pointcloud.shape,angle.shape)
            pointcloud = torch.cat((pointcloud, angle), dim = -1)
            scanid = torch.zeros(pointcloud.shape[0] * pointcloud.shape[1], 1,dtype=pointcloud.dtype,device=pointcloud.device)
            # scanid = torch.zeros_like(angle)
            # print(pointcloud.shape,angle.shape,scanid.shape, pointcloud)
            print(pointcloud[:, :, 4] >= -8.83, pointcloud[pointcloud[:, :, 4] >= -8.83][:,4]*3)
            scanid[pointcloud[:, :, 4] >= -8.83] = int(
                        (2 - (pointcloud[pointcloud[:, :, 4] >= -8.83][:, 4])) * 3.0 + 0.5)
            # print(scanid[scanid!=0])
            scanid[pointcloud[:, :, 4] < -8.83] = 32 + int(
                        (-8.83 - (pointcloud[pointcloud[:, :, 4] < -8.83][:, 4])) * 2.0 + 0.5)
            # Counter(scanid)
            scanid = scanid.view(-1,32,1)
            print(scanid)
            # pointcloud = np.insert(pointcloud, 5, values=scanid, axis=1)
            features_ls.append(scanid)
